...
#[derive(Clone, Debug)]
pub struct ZSpiderDecomp { vert: V, alpha: f32 }
impl<G: GraphLike> Decomposition<G> for ZSpiderDecomp {
    fn update(&mut self, g: &G) {
        if let Some(v) = g.vertices().max_by(|s, t| usize::cmp(&g.tri_degree(*s), &g.tri_degree(*t))) {
            self.vert = v;
...
impl ZSpiderDecomp {
    pub fn new() -> ZSpiderDecomp {
        ZSpiderDecomp { vert: 0, alpha: f32::INFINITY }
    }
}
...
/// Decompose triangles
#[derive(Clone, Debug)]
pub struct TriDecomp { max_tris: usize, edges: Vec<(V, V)>, alpha: f32 }
...
impl TriDecomp {
    pub fn new(max_tris: usize) -> TriDecomp {
        TriDecomp { max_tris: max_tris, edges: vec![], alpha: f32::INFINITY }
    }
}
...
impl<G: GraphLike> Decomposition<G> for TriStateDecomp {
    fn update(&mut self, g: &G) {
        ...
        for v in g.vertices() {
            if g.vertex_type(v) == VType::Z && g.degree(v) == 1 && g.incident_edges(v).next().unwrap().1 == EType::T {
                let phase = g.phase(v);
    ...
    fn alpha(&self, _: &G) -> f32 {
        match max(self.zeros.len(), max(self.pi_over_2s.len(), self.minus_pi_over_2s.len())) {
    ...
    fn apply(&self, g: &G) -> Vec<G> {
        let phase = 
            if self.zeros.len() == 3 { Phase::Zero } 
            else if self.pi_over_2s.len() == 3 { Phase::PiOver2 }
            else { Phase::MPiOver2 };
        ...
        *g1.scalar_mut() *= match phase {
            ...
            Phase::PiOver2 => 
                ...
                (ScalarN::one() + (ScalarN::sqrt2_pow(2) + ScalarN::one()) * ScalarN::from_phase(Rational::new(1,2))) * ScalarN::sqrt2_pow(-2),
            Phase::MPiOver2 => 
                // (1 + (-1)*(2^1 + 1)*exp(i*pi/2)) * sqrt(2)^-2 = (1 - 3i) / 2
                (ScalarN::one() + ScalarN::minus_one() * (ScalarN::sqrt2_pow(2) + ScalarN::one()) * ScalarN::from_phase(Rational::new(1,2))) * ScalarN::sqrt2_pow(-2),
        };
        ...
        *g2.scalar_mut() *= match phase {
            ...
            Phase::PiOver2 =>
                ...
                (ScalarN::one() + ScalarN::minus_one()*ScalarN::from_phase(Rational::new(1,2))) * ScalarN::sqrt2_pow(-2),
            Phase::MPiOver2 =>
        ...
        *g3.scalar_mut() *= match phase {
            ...
            Phase::PiOver2 | Phase::MPiOver2 =>
                ...
                ScalarN::minus_one() * (ScalarN::sqrt2_pow(2) + ScalarN::one() + ScalarN::from_phase(Rational::new(-1,2))) * ScalarN::sqrt2_pow(-3)
        };
        ...
        *g4.scalar_mut() *= match phase {
            Phase::Zero =>
                ...
                ScalarN::minus_one() * (ScalarN::sqrt2_pow(2) + ScalarN::one()) * ScalarN::sqrt2_pow(-3),
            Phase::PiOver2 =>
...
impl TriStateDecomp {
    pub fn new(use_zeros: bool, use_pi_over_2s: bool, use_minus_pi_over_2s: bool) -> TriStateDecomp {
        TriStateDecomp { 
            zeros: vec![],
            pi_over_2s: vec![],
...
#[derive(Clone, Debug)]
pub struct CatDecomp { cat_nodes: Vec<V> }
impl<G: GraphLike> Decomposition<G> for CatDecomp {
    fn update(&mut self, g: &G) {
        ...
        for v in g.vertices() {
            ...
            if g.phase(v).denom() != &1 || !g.incident_edges(v).all(|(w, ety)| ety == EType::H && g.phase(w).denom() == &4)  {
                continue;
            }
    ...
    fn apply(&self, g: &G) -> Vec<G> {
        ...
        if cat_nodes[1..].len() == 6 {
            return vec![CatDecomp::cat6_0(&g, &cat_nodes), CatDecomp::cat6_1(&g, &cat_nodes), CatDecomp::cat6_2(&g, &cat_nodes)];
        } else if cat_nodes[1..].len() == 4 {
            return vec![CatDecomp::cat4_0(&g, &cat_nodes), CatDecomp::cat4_1(&g, &cat_nodes)];
        }
...
impl<G: GraphLike> Decomposition<G> for TDecomp {
    ...
    fn apply(&self, g: &G) -> Vec<G> {
        match self.ts.len() {
            ...
            2 | 3 | 4 => vec![TDecomp::bell_s(g, &self.ts[0..2]), TDecomp::epr(g, &self.ts[0..2])],
            5 => vec![TDecomp::magic5_0(g, &self.ts), TDecomp::magic5_1(g, &self.ts), TDecomp::magic5_2(g, &self.ts)],
            6 => vec![TDecomp::b60(g, &self.ts), TDecomp::b66(g, &self.ts), TDecomp::e6(g, &self.ts), TDecomp::o6(g, &self.ts), TDecomp::k6(g, &self.ts), TDecomp::phi1(g, &self.ts), TDecomp::phi2(g, &self.ts)],
...
impl TriDecomposer {
    ...
    pub fn for_graph<G: GraphLike>(g: &G) -> TriDecomposer {
        ...
        if has_tris {
            ...
            decomposer.add_decomp(Decompositions::TriState(TriStateDecomp::new(true, true, true)));
        }
    ...
    fn find_best_decomp(decomps: Vec<Decompositions>, g: &impl GraphLike) -> Decompositions {
        decomps.into_iter()
            .map(|d| { let mut d = d; d.update(g); d })
            .min_by(|d1, d2| d1.alpha(g).partial_cmp(&d2.alpha(g)).unwrap_or(std::cmp::Ordering::Equal))
            .unwrap()
    }
    ...
    pub fn decomp_parallel(&self, g: impl GraphLike) -> (ScalarN, usize) {
        ...
        if best_decomp.alpha(&g) < f32::INFINITY {
            ...
            //println!("{:?}", best_decomp);
            let result = best_decomp.apply(&g).into_par_iter().map(|dg | {
                let mut dg = dg;
                self.simplify(&mut dg);
            ...
            }).reduce_with(|(s1, t1), (s2, t2)| (s1+s2, t1+t2)).unwrap();
            ...
            if self.debug {
                ...
                if result.0 != expected {
                    ...
                    std::fs::write("graph.dot", before.unwrap().to_dot()).expect("Unable to write file");
