...
pub trait GraphLike: Clone + Sized + Send + Sync + std::fmt::Debug {
    ...
    fn tri_edges(&self) -> TriEdgeIter {
        TriEdgeIter { eiter: self.edges() }
    }

    fn tri_degree(&self, v: V) -> usize {
        return self.incident_edges(v).filter(|(_, ety)| *ety == EType::T).count();
    }
            ...
            match (st, tt) {
                (VType::Z, VType::Z) | (VType::X, VType::X) => {
                    match (ety0, ety) {
                        ...
                        (EType::T, EType::T) | (EType::H, EType::T) | (EType::T, EType::H) => {
                            if st != VType::Z { panic!("Parallel star edges between X vertices not supported") }
                        ...
                        (EType::T, EType::N) | (EType::N, EType::T) => {
                            if st != VType::Z { panic!("Parallel star and normal edge between X vertices not supported") }
                ...
                (VType::Z, VType::X) | (VType::X, VType::Z) => {
                    match (ety0, ety) {
                        ...
                        _ => { panic!("This parallel edge involving star is not suported") }
                    }
                }
    ...
    fn to_dot(&self) -> String {
        ...
        for (s, t, ty) in self.edges() {
            ...
            if ty == EType::T { dot += " [color=orange]"; }
            dot += "\n";
        }
    ...
    fn from_dot(dot: &str) -> Self {
        ...
        for line in dot.split("\n") {
            ...
            if !line.contains("--") {
                ...
                let phase = if data[1].contains(':') {Rational::one()} else {Rational::zero()};
    ...
    fn triangle_encode_ts(&mut self) {
        let ts: Vec<V> = self.vertices().filter(|&v| self.phase(v).denom() == &4).collect();
        if ts.len() < 2 {
            return;
        }
