...
use crate::circuit::*;
use crate::gate::*;
use rand::{SeedableRng, Rng, seq::IteratorRandom};
...
impl RandomCircuitBuilder {
    ...
    pub fn build(&mut self) -> Circuit {
        ...
        for _ in 0..self.depth {
            ...
            let qs = (0..self.qubits).choose_multiple(&mut self.rng, 3);
            ...
            p0 += self.p_ccz;
            if p < p0 { c.push(Gate::new(CCZ, vec![q0,q1,q2])); continue; }
        }
...
impl RandomHiddenShiftCircuitBuilder {
    ...
    pub fn n_cswap(&mut self, n_cswap: usize) -> &mut Self { self.n_cswap = n_cswap; self }
    ...
    fn random_cswap(&mut self, c: &mut Circuit) -> (usize, usize, usize) {
        let qs = self.qubits/2;
        let mut q0 = self.rng.gen_range(0..qs);
        let mut q1 = self.rng.gen_range(0..qs-1);
        let mut q2 = self.rng.gen_range(0..qs-2);
        ...
        if q2 >= q0 { q2 += 1; }
        if q2 >= q1 { q2 += 1; }
        c.push(Gate::new(CNOT, vec![q0,q2,q1]));
        c.push(Gate::new(TOFF, vec![q0,q2,q1]));
        c.push(Gate::new(CNOT, vec![q0,q2,q1]));
        (q0, q1, q2)
    }
    ...
    pub fn build(&mut self) -> (Circuit, Vec<u8>) {
        ...
        let mut cswaps = Vec::new();
        for _ in 0..self.n_cswap {
            self.random_clifford_layer(&mut oraclef);
            let cswap = self.random_cswap(&mut oraclef);
            cswaps.push(cswap);
        }
        ...
        // Undo CSWAPs
        cswaps.reverse();
        for (q0,q1, q2) in cswaps {
            oraclef.push(Gate::new(CNOT, vec![q0,q2,q1]));
            oraclef.push(Gate::new(TOFF, vec![q0,q2,q1]));
            oraclef.push(Gate::new(CNOT, vec![q0,q2,q1]));
        }
