...
pub fn spider_fusion(g: &mut impl GraphLike, v0: V, v1: V) -> bool {
    if check_spider_fusion(g, v0, v1) {
        spider_fusion_unchecked(g, v0, v1); true
    } else { false }
}
...
pub fn remove_id_unchecked(g: &mut impl GraphLike, v: V) {
    ...
    let new_et =
        match (nhd[0].1, nhd[1].1) {
            ...
            (EType::T, EType::H) | (EType::H, EType::T) => panic!("Cannot remove identity between star and hadamard edge"),
...
pub fn check_color_change(g: &impl GraphLike, v: V) -> bool {
    ...
    (vt == VType::X || vt == VType::Z) &&
        g.incident_edges(v).all(|(_,et)| et == EType::N || et == EType::H)
}
...
pub fn tri_edge_unchecked(g: &mut impl GraphLike, v: V) {
    ...
    match g.vertex_type(v) {
        VType::Z => {
            ...
            } else { panic!("tri_edge: Unexpected phase"); }
        }
        VType::X => {
            if phase.is_zero() {
...
pub fn split_neighbor_unchecked(g: &mut impl GraphLike, v: V) {
    ...
    let new_vty = match g.edge_type(v, w) {
        ...
        EType::H => if g.vertex_type(v) == VType::X { VType::Z } else { VType::X }
...
checked_rule1!(check_split_neighbor, split_neighbor_unchecked, split_neighbor);
    ...
    if et == EType::T {
        if t0 == VType::Z && t1 == VType::Z {
            ...
            *g.scalar_mut() *= ScalarN::one() + ScalarN::from_phase(p0) + ScalarN::from_phase(p1);
            ...
            let mut s = 
                ScalarN::one() + ScalarN::one() + ScalarN::one() + 
