...
impl Circuit {
    ...
    pub fn add_qubit(&mut self) { self.nqubits += 1; }
    ...
    pub fn add_gate_with_param(&mut self, name: &str,
        qs: Vec<usize>, param: &String)
    {
        self.push(Gate {
        t: GType::from_qasm_name(name),
        qs,
    ...
    pub fn to_graph_with_options<G: GraphLike>(&self, postselect: bool, use_star_edges: bool, remember_params: bool, param_map: &mut FxHashMap<String, Vec<V>>) -> G {
        let mut graph = G::new();
        ...
        for g in &self.gates {
            let res = g.add_to_graph(&mut graph, &mut qs, postselect, use_star_edges);
            if remember_params {
                if let Some((param, vert)) = res {
                    param_map.entry(param).or_insert(Vec::new()).push(vert)
                }
            }
        }
    ...
    pub fn to_graph<G: GraphLike>(&self) -> G {
        return self.to_graph_with_options(false, false, false, &mut FxHashMap::default());
    }
...
mod tests {
    ...
    fn tograph_postsel() {
        let c = Circuit::from_qasm(r#"
            ...
            ccz q[0], q[1], q[2];
        "#).unwrap();
        let g: Graph = c.to_graph_with_options(true, false, false, &mut FxHashMap::default());
        assert_eq!(c.to_tensor4(), g.to_tensor4());
