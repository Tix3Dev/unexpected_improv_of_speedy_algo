...
pub fn color_change_simp(g: &mut impl GraphLike) -> bool {
    vertex_simp(g, check_color_change, color_change_unchecked, false)
}

pub fn split_neighbor_simp(g: &mut impl GraphLike) -> bool {
    vertex_simp(g, check_split_neighbor, split_neighbor_unchecked, false)
}

pub fn tri_edge_simp(g: &mut impl GraphLike) -> bool {
    vertex_simp(g, check_tri_edge, tri_edge_unchecked, false)
}
...
pub fn fuse_gadgets(g: &mut impl GraphLike) -> bool {
    ...
    'outer: for v in g.vertices() {
        if g.degree(v) == 1 && g.vertex_type(v) == VType::Z {
            ...
            if g.vertex_type(w) != VType::Z || !g.phase(w).is_zero() || g.edge_type(v, w) != EType::H { continue; }
            let mut nhd = Vec::new();
            for (n,et) in g.incident_edges(w) {
                if et == EType::T { continue 'outer }
    ...
    let gadgets = g.vertices()
        ...
        .filter(|&(n, v)| {
            g.edge_type(v, n) == EType::H
                ...
                && g.phase(n).is_one()
                // No triangle edges
                && g.incident_edges(n).all(|(_, ety)| ety != EType::T)
        })
